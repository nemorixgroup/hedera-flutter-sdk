...
void main() {
  group('TokenDissociateTransaction', () {
    // ---- default values ----

    group('default values', () {
      test('accountId defaults to null', () {
        final tx = TokenDissociateTransaction();
    ...
    group('toBytes serialization', () {
      test('encodes the account', () {
        ...
        final body = TokenDissociateTransactionBody.fromBuffer(tx.toBytes());
    ...
    group('applyToBody / buildBody', () {
      test('sets tokenDissociate on the TransactionBody', () async {
        final tx = TokenDissociateTransaction()
            .setAccountId(AccountId.fromString('0.0.12345'))
            .addTokenId(TokenId.fromString('0.0.999'));
        ...
        expect(
          body.whichData(),
          equals(TransactionBody_Data.tokenDissociate),
        );
        expect(body.tokenDissociate.tokens.length, equals(1));
      });
    ...
    group('integration', () {
      ...
      test('fluent API can chain Transaction and subclass setters', () {
        ...
        final tx = TokenDissociateTransaction()
            ...
            .addTokenId(tokenId)
            .setMemo('dissociate memo');

        expect(tx.accountId, equals(accountId));
        expect(tx.tokenIds, equals([tokenId]));
        expect(tx.memo, equals('dissociate memo'));
      });
    });
  });
}
